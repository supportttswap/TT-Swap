...
namespace BeCoreApp.Application.Implementation
{
    public class BlogService : IBlogService
    {
        ...
        {
            return _blogRepository.FindAll(c => c.Status == Status.Active)
                .OrderByDescending(x => x.Id).Take(top)
        ...
        public PagedResult<BlogViewModel> GetAllPaging(string startDate, string endDate, string keyword, int blogCategoryId, int pageIndex, int pageSize)
        {
            ...
            if (blogCategoryId != 0)
            {
                var blogCategoryList = _blogCategoryService.GetBlogCategoryIdsById(blogCategoryId, MenuFrontEndType.Home);
            ...
            var data = query.OrderByDescending(x => x.Id).Skip((pageIndex - 1) * pageSize).Take(pageSize)
                .Select(x => new BlogViewModel
                {
                    ...
                    DateCreated = x.DateCreated,
                    ViewCount = x.ViewCount,
                    BlogTags = x.BlogTags.Select(bt => new BlogTagViewModel
                    {
                        Id = bt.Id,
                        BlogId = bt.BlogId,
                        TagId = bt.TagId
